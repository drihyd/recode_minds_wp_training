...
$(document).on('ready', function() {
	...
	if(typeof initAdjustPeopleGrid !== 'undefined') {
		adjustPeopleGrid();
	}
	
	if(typeof initAdjustPeopleinfoGrid !== 'undefined') {
		adjustPeopleinfoGrid();
	}
	
	if(typeof initAdjustBlogpostsGrid !== 'undefined') {
		adjustBlogpostsGrid();
	}
	
	if(typeof initAdjustClientworkGrid !== 'undefined') {
		adjustClientworkGrid();
	}
});
...
function adjustSolutionsGrid() {
	if($("#mqCheck-649").is(':visible')) {
		$('.solutions-grid .item .wrapper').css('height', 'auto');
		
		$('.solutions-grid').each(function() {
			var heights = [];
			var grid = $(this);
...
function adjustPeopleGrid() {
	if($("#mqCheck-649").is(':visible')) {
		$('.people-grid .item .wrapper').css('height', 'auto');
		
		$('.people-grid').each(function() {
			var heights = [];
			var grid = $(this);
			var wrappers = $(grid).find('.wrapper');
...
function adjustPeopleinfoGrid() {
	if($("#mqCheck-499").is(':visible')) {
		$('.peopleinfo-grid .item .wrapper').css('height', 'auto');
		
		$('.peopleinfo-grid').each(function() {
			var heights = [];
			var grid = $(this);
			var wrappers = $(grid).find('.wrapper');
...
function adjustBlogpostsGrid() {
	if($("#mqCheck-767").is(':visible')) {
		$('.blogs-grid .item .wrapper').css('height', 'auto');
		
		$('.blogs-grid').each(function() {
			var heights = [];
			var grid = $(this);
			var wrappers = $(grid).find('.wrapper');
...
$(".peopleinfo-know-more").click(function(e) {
	e.preventDefault();
	var elem = $(this);
	var image = $(elem).parents('.wrapper').find('.dp').attr('style');
	var title = $(elem).parents('.wrapper').find('.details .title').html();
	var socialLinks = $(elem).parents('.wrapper').find('.dp .summary-card .social-links').html();
	var text = $(elem).parents('.wrapper').find('.dp .summary-card p').html();
	
	var htmlContent = '<div class="dp" style="' + image + '"></div>';
	htmlContent += '<h3 class="title">' + title + '</h3>';
	htmlContent += '<ul class="social-links list-inline">' + socialLinks + '</ul>';
	htmlContent += '<p>' + text + '</p>';
	console.log(htmlContent);
	
	var modal = $("#peopleinfoModal");
...
function adjustClientworkGrid() {
	if($("#mqCheck-649").is(':visible')) {
		$('.clientwork-grid .item .wrapper').css('height', 'auto');
		
		$('.clientwork-grid').each(function() {
			var heights = [];
			var grid = $(this);
			var wrappers = $(grid).find('.wrapper');
...
$('.main-menu-toggle').click(function(e) {
	e.preventDefault();
	$('.main-menu-mobile').toggleClass('showmenu');
...
$('.height-restrict-trigger').click(function(e) {
	e.preventDefault();
	
	var elem = $(this);
	var parent = $(elem).parent('.height-restricted');
...
var highlightActivePage = function() {
	...
	$("#main-sidebar .list-group-item.active").each(function() {
		var elem = $(this);
		console.log(elem);
		var secondSubMenu = elem.parent(".list-group-submenu-2");
		secondSubMenu.addClass("in");
		secondSubMenu.removeClass("collapse");
		
		var firstSubMenu = elem.parent(".list-group-submenu");
		firstSubMenu.addClass("in");
		firstSubMenu.removeClass("collapse");
	});
};
...
$('.collapse-wrapper .dropper').click(function(e) {
	e.preventDefault();
	var elem = $(this);
