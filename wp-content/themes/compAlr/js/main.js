...
// TEXT CHANGE :BEGIN 
function func_textChanger() {

	var id = $('#changingword');
	var words = ['Analyse','Identify','Monitor'], i = 0;
...
// IMAGE CHANGE :BEGIN 
function func_imageChanger() {

	var myImage = $('#mainImg');
	var imageArray = ["./img/analyse-icon.png","./img/identify-icon.png","./img/monitor-icon.png"], imageIndex = 0;
	var imageAlt = ["Analyse","Identify","Monitor"], altIndex = 0;
	setInterval(function(){
		myImage.fadeOut(function(){
		$(this).attr("src",imageArray[imageIndex=(imageIndex+1)%imageArray.length]).fadeIn();
		$(this).attr("alt",imageAlt[altIndex=(altIndex+1)%imageAlt.length]).fadeIn();
		});
	}, 5000);
	return false;
};
...
$(".get-more-toggle").click(function(e) {
    e.preventDefault();
    var elem = $(this);
    if(elem.hasClass('open')) {
        $("#get-more-overlay").removeClass('showoverlay');
        $(".get-more-slide").removeClass('show');
        $('.get-more-toggle').removeClass('open');
    } 
    else {
        $(".get-more-slide").addClass('show');
        $('.get-more-toggle').addClass('open');
        $("#get-more-overlay").addClass('showoverlay');
    }
});

function adjustFeaturesGrid() {
	if($("#mqCheck-649").is(':visible')) {
		$('.grid-slider .item .wrapper').css('height', 'auto');
